...
cv::Mat FrameProcessor::process(const cv::Mat& frame, const AppState& state) {
    ...
    // Передаємо кадр на аналіз у фоновий потік FaceDetector
    faceDetector_.updateFrame(adjusted);

    cv::Mat result;
    switch (state.mode) {
        case ProcessingMode::GLITCH:   
            result = applyGlitch(adjusted);   
            break;
            
        case ProcessingMode::PIXELATE: 
            result = applyPixelate(adjusted); 
            break;
            
        case ProcessingMode::FACE: { // Додали фігурну дужку для ізоляції змінних
            result = adjusted.clone(); // Виправлено: тепер копія зберігається в result
            
            // Забираємо з фонового потоку останні відомі рамки облич
            std::vector<cv::Rect> faces = faceDetector_.getLatestFaces();
            
            // Малюємо рамки на поточному кадрі
            for (const auto& faceRect : faces) {
                cv::rectangle(result, faceRect, cv::Scalar(0, 255, 0), 2, cv::LINE_AA);
                
                // Рендеримо невелику плашку над рамкою
                cv::putText(result, "AI Face", cv::Point(faceRect.x, std::max(faceRect.y - 5, 15)),
...
void FrameProcessor::drawHUD(cv::Mat& frame, const AppState& state) const {
    ...
    std::string help = "[0] Normal  [1] Glitch  [2] Pixelate [F] Face [D] Draw  [Tab] cycle  [X] clear  [slider] brightness  [Q] quit";
